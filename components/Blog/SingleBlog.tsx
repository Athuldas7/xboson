...
const SingleBlog = ({ blog }: { blog: Blog }) => {
  const { id, title, image, paragraph, author, tags, publishDate, subpara } = blog;

  return (
    <>
      <div
        className="flex flex-col justify-between h-full overflow-hidden bg-white rounded-md wow fadeInUp shadow-one dark:bg-dark"
        data-wow-delay=".1s"
      >
        <Link href={`/blog/${id}`} className="relative block w-full" style={{ paddingTop: '60%' }}>
          ...
          <Image
            ...
            style={{ width: '100%', height: '100%' }} // Ensure the image scales properly
          />
        </Link>
        <div className="flex flex-col justify-between p-6 sm:p-8 md:py-8 md:px-6 lg:p-8 xl:py-8 xl:px-5 2xl:p-8">
          <div>
            <h3>
              <Link
                href={`/blog/${id}`}
